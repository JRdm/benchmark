...
def setup_cluster(conn, master_nodes, slave_nodes, ambari_nodes, OPTS, deploy_ssh_key, cluster_name):
  ...
  print "Setting up ambari agent on slaves..."
  for slave in slave_nodes:

    print "Slave: %s" % slave.public_dns_name
    setup_ambari_slave(ambari, slave, OPTS)
...
def setup_ambari_slave(ambari, slave, OPTS):
  cmd = """
        wget http://public-repo-1.hortonworks.com/ambari/centos6/1.x/updates/1.6.0/ambari.repo;
        cp ambari.repo /etc/yum.repos.d;
        yum -y install epel-release;
        ...
        sed -i.bak "/^hostname/ s/.*/hostname=%s/" /etc/ambari-agent/conf/ambari-agent.ini;
        ambari-agent start;
